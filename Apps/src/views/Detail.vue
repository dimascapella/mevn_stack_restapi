<template>
    <div class="min-h-screen flex justify-center items-center font-mono">
        ...
        <p class="text-center text-3xl font-bold">Edit User Data</p>
...
export default {
    ...
    mounted() {
        ...
        axios.get(`http://localhost:5000/user/${this.route.params.id}`)
        .then((result) => {
    ...
    methods: {
        updateData: function () {
            ...
            axios.put(`http://localhost:5000/user/${this.route.params.id}`, data)
            .then(() => {
                this.router.push({
                    name: 'index'
                })
